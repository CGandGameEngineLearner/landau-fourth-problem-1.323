...
的二素数后代。Merikoski 已指出可在这里继续使用 Buchstab 恒等式产生额外
...
\begin{remark}
同一源代码在 GCC 未定义行为检查器下编译时给出相同结果。更细的
$1800\times400\times220$ 与 $2400\times500\times280$ 网格分别给出下界
$0.033508163100$ 与 $0.034213110567$。这些数值只支持收敛性判断，不进入定理。
\end{remark}
...
\cite[PDF 第 15--16 页，定理 2 的证明]{Merikoski2020}。
...
\]
小型审计模块检查 $\nu$ 的仿射恒等式、在 $7/6<\alpha<1.22$ 上满足这些恒等式
的显式有理值 $\nu=10^{-20}$、半整数切点、抽象表示数界、不利方向格子几何、
$\alpha/\gamma,t+1,\Phi,U_{\rm LS}$ 的代数归一化、$s<1$ 切点恒等式
$(2/s)/((1-\omega)\log D)=2/\log D$，并检查
\[
 \frac{2994511739}{18000000000000},\qquad
 10^{11}\nu<\frac{2994511739}{18000000000000}.
\]
它还检查在所述假设下的有限 Perron 预算算术
$\log((V-1/2)/U)\ge 2/(4U+1)$ 与
$X^{B}\cdot 2\cdot X^{C-A'}\le X^{-A}$。
它们不构造真实 Perron 局部化或随前缀变化的 Rosser 权，也不证明完整解析定理。
形式化仍把 Grimmelt--Merikoski Corollaries 7.1/7.2 和标准维数一线性筛定理
作为外部解析输入；第~\ref{sec:transfer-zh}、\ref{sec:switching-zh} 节在论文中
核对其假设并证明有限 Perron/Mellin 局部化，但这些解析论证不是 Lean 定理。
...
Lean 只核查有限组合、整数、取整、初等 $F_6$、有理端点和实幂换算；所引用的
Type I/II 与线性筛定理仍是外部输入。有限 Mellin--Perron 传递在论文中证明，
但未在 Lean 中形式化。
\end{remark}
...
解析。固定宽度 C++ 证书与任意精度 Python 审计的命令行均为
\texttt{1200 300 160}，并分别打印
\[
 \texttt{saving\_lower=0.032303187971},\qquad
 \texttt{CERTIFIED=YES}.
\]
外部 PDF 仅记录来源与 SHA-256，不在仓库中重新分发。
\end{remark}
