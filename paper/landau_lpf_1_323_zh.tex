...
\]
基项、三素数密度和完整二素数密度的变化分别不超过
$10^6t$、$4\cdot10^8t$、$5\cdot10^8t$。最后一个界也覆盖递归候选：
$u_+/\gamma_t$ 的变化小于 $10^4t$，其内部
$W_3/\beta_3^2$ 积分的变化小于 $2\cdot10^5t$。取最大或最小不会增大候选中
最大的 Lipschitz 常数。
...
\]
事实上，写 $q_i=Q_iv_i$、$k=Pw$ 后，每个光滑因子都来自归一化变量
$v_i,w$ 上同一个固定紧支撑函数族，其 $r$ 阶导数一致为 $O_r(1)$。对
$p<z=q_j$ 有 $p\nmid u$，所以局部密度严格满足 $g_u(p)=g(p)$。因此第
11--12 章使用的维数一乘积条件
\[
 \prod_{w\le p<z}(1-g_u(p))^{-1}
 \le\frac{\log z}{\log w}
 \left(1+O\!\left(\frac1{\log w}\right)\right)
\]
与无前缀时具有同一隐含常数。基本引理常数只依赖该乘积界和固定导数半范数，
不依赖最小前缀素数在 dyadic 格中的具体取值。这证明了 $z=q_j$ 时所需的一致性。
