...
\title{关于 $n^2+1$ 的最大素因子的\\递归 Buchstab 切换改进\\
\large 中文译本}
\author{JinWen Li}
\date{2026年8月29日}
...
\begin{center}
\small
西南石油大学\quad\texttt{lifesize1@163.com}\\[4pt]
...
\noindent{\small 2020 MSC：11N32, 11N35, 11N75.}
...
$1.317$ \cite{Li2025}。\footnote{本文不采用 Carella 的孤立 $3/2$ 主张：其
(5.3)--(5.4) 把完整 von Mangoldt 除数恒等式换成
$\log\operatorname{rad}(m)$，遗漏了素数幂贡献 \cite[Section 5]{Carella2025}。}
...
的二素数后代；其思路属于 Harman 筛法 \cite{Harman2007}。Merikoski 已指出可在这里继续使用 Buchstab 恒等式产生额外
Type II 和，但在其发表计算中未使用这些和 \cite[Remark 12]{Merikoski2020}。
Li 的 $1.317$ 论证保持原筛分解，仅改变积分数值
\cite[Lemmas 2.1 and 2.2]{Li2025}，因此本文的三素数项并不包含在该结果中。

\begin{theorem}\label{thm:main-zh}
假设第~\ref{sec:transfer-zh} 节与第~\ref{sec:switching-zh} 节的
Grimmelt--Merikoski Type I/II 估计、维数一线性筛以及有限 Mellin--Perron
传递成立。则存在无穷多个正整数 $n$，使得
\[
...
\begin{remark}[证明状态边界]
定理中的假设即为上述解析输入，它们未在 Lean 中形式化。Lean 只核查有限代数、
格子几何、整数证书、端点和实幂换算，既未验证这些解析输入，也未解决
Landau 第四问题。
\end{remark}
...
\begin{proof}
在 Grimmelt--Merikoski 中取 $a=h=1$。其 Theorem 1.1 后、PDF 第 2 页的
段落说明：当 $ah\le x^\varepsilon$ 时，辅助素数密度条件由经典零点自由区域
无条件推出，可能的例外零点只会使相应和更小。因此本文固定平移不再需要额外
特征假设。

PDF 第 22 页的 Corollary 7.1 在 $D\le x^{1/2-\eta}$ 上控制完整光滑模
$d$ 偏差绝对值之和。其测试函数归一化支撑于 $[1,2]$ 与 $[-1,1]$，常数只依赖
有限个导数半范数。本文每个 dyadic 块均由同一固定光滑分片缩放得到，故这些
半范数及隐含常数对块一致；$O((\log x)^L)$ 个块和下文 Mellin 核只损失对数幂。

PDF 第 22--23 页的 Corollary 7.2 对除数有界系数成立，要求 $N$ 侧支撑于
平方自由数，并给出
\[
 x^{\alpha-1+2\eta}\le N
 \le x^{(2-\alpha)/3-4\eta/3}.
\]
本文每次取 $N$ 为至多三个互异前缀素数的子集积，故 $N$ 平方自由且有序表示数
至多为 $\tau_3(N)$。其余前缀素数、Möbius 变量及余因子使 $M$ 侧系数至多为
$\tau_5(M)$。局部化相位在竖线上实部有界、虚部模为 $1$，所以有限
Mellin--Perron 分离后仍保持这些固定除数界。又 $N\le x^{1/3}$，故
$M=x^\alpha/N\ge N$。固定正 $\eta$ 时上式正是
\eqref{eq:typeII-zh} 的闭内区间；反之任意固定闭内区间都可由足够小的 $\eta$
获得。
...
\]
无条件情形 $\theta=7/64$。固定筛余量后取辅助 $\varepsilon=\eta$，分别得到
$1-17\eta/32$ 与 $1-21\eta/32$，足以吸收对数损失。PDF 第 23 页
Corollary 7.2 后的段落明确指出，这正是 Merikoski 定理 2 证明所用的
Type I/II 区间。至此已逐项核对两条推论的假设；推论本身仍作为引用的外部结果。
\end{proof}
...
\[
 ...
 <x^{\sigma-4\nu/3}.
\]
使用引理~\ref{lem:explicit-margin-zh} 的固定值
$\nu=\nu_0=10^{-20}$。证书只保留 $\alpha<1.22$；此时
$4\nu_0<1/25\le\gamma$ 且 $5\nu_0<1/25\le\sigma-a$，故
$0<\gamma_{\nu_0}<\tau_{\nu_0}<a<\sigma$。所有解析估计都使用这一固定正余量，
端点论证不再令 $\nu$ 取未量化的极限。
...
\begin{proof}
因 $U,V$ 为整数，$U<V$ 等价于 $U<V-1/2$。令
$y=(V-1/2)/U$、$c=1/\log x$。对 $y\ne1$，截断 Perron 公式为
\[
 \frac{1}{2\pi i}\int_{c-iT}^{c+iT}y^s\frac{ds}{s}
 =\1_{y>1}+O\!\left(y^c
 \min\left\{1,\frac1{T|\log y|}\right\}\right).
\]
具体估计如下。完整竖线积分在 $y>1$ 时向左闭合并取 $s=0$ 的留数，等于 $1$；
在 $0<y<1$ 时向右闭合，等于 $0$。对省略的两条竖直尾，在
$e^{it\log y}/(c+it)$ 中分部积分；当 $T|\log y|\ge1$ 时得到
$O(y^c/(T|\log y|))$，否则用完整过渡的平凡界 $O(y^c)$。这证明上式。

半整数给出一致间隔。若 $U<V$，则 $y\ge1+1/(2U)$，从而
\[
 \log y\ge\frac2{4U+1}.
\]
若 $U\ge V$，则 $1-y\ge1/(2U)$ 且
$-\log y\ge1-y$。所以两种情形均有
\[
 |\log y|\ge\frac2{4U+1}\ge\frac2{5x^C}.
\]
又在 $y>1$ 时 $y^c\le e^C$，在 $y<1$ 时 $y^c\le1$，故一次比较的
逐点截断误差为 $O_C(x^C/T)$。半整数只排除相等情形，不改变表示数和除数界。
$U^{-s}$ 与
...
对第一个合格子集 $I$，令
$N=\prod_{i\in I}q_i$、
$M=(\prod_{i\notin I}q_i)ds$。跨两侧的次序条件把较小素数的
$U^{-s}$ 相位放到其所在侧，把较大素数的 $(V-1/2)^s$ 相位放到另一侧。
若 $q_3\in N$，则 $P^+(d)^{-s}$ 放在 $M$ 侧、
$(q_3-1/2)^s$ 放在 $N$ 侧；若 $q_3\notin N$，粗糙性完全位于 $M$ 侧。

\item[直接二素数 Type II：]
取 $N=q_1q_2$，把筛余因子与 Möbius 变量放在 $M$ 侧。
$q_2<q_1$ 在 $N$ 侧内部；当 $d>1$ 时，唯一跨侧条件为
$P^+(d)<q_2$，按上述半整数 Perron 相位分配。$d=1$ 自动成立。

\item[Type I 前缀：]
$A_3$ 下筛的前缀为 $q_1q_2q_3$、切点为 $q_3$；通用
$U_{\rm LS}$ 分支的前缀为 $q_1q_2$、切点为 $q_2$。次序条件在前缀内部，
可变切点由命题~\ref{prop:prefix-sieve-zh} 的前缀相关 Rosser 权携带；这两类
分支不使用本 Type II Perron 引理。

\item[固定切点基项：]
一素数尾项基项的前缀为 $q_1$，递归基项的前缀为 $q_1q_2$，两者切点均为
$x^{\gamma_\nu}$。格子条件在应用基本命题前已固定；直接上界与递归上界的选择
也是逐盒决定，不是新的 Perron 条件。
\end{description}

在 $\Re s=c$ 上实部因子至多为 $e^C$，虚部因子模为 $1$，且当
$T=x^{A'}$ 时
\[
 \int_{-T}^{T}\frac{dt}{|c+it|}
 \ll\log(T\log x)\ll_{A'}\log x.
\]
把 $J$ 个指标逐一替换并作望远镜求和。若原分解至多有 $x^B$ 个元组，则总误差
为
\[
 O_{C,J}\!\left(x^B\frac{x^C}{T}(\log x)^{J-1}\right).
\]
给定 $A>0$，取 $A'>A+B+C+J+2$ 即得 $O_A(x^{-A})$。多重积分核的总
$L^1$ 质量为 $O((\log x)^J)$，故一致 Type II 误差
$O(x^{1-\delta})$ 只变为 $O(x^{1-\delta}(\log x)^J)
=O(x^{1-\delta/2})$。
光滑产品权由 Mellin 变换分离，$\log(MN)=\log M+\log N$ 由分部求和处理。
该机制以 \cite[Sections 2.3 and 3.4]{Merikoski2020} 为模板。这是通常的解析
证明，不属于 Lean 形式化范围。
\end{proof}
...
本命题不是 Lean 定理。外部输入只有两项：Grimmelt--Merikoski 推论 7.1；
以及 \emph{Opera de Cribro} 的维数一基本引理。本文不重证这两个源定理。附录
核对最小前缀切点下的全部假设，包括一致 Euler 乘积、归一化光滑函数族和收集后
的绝对余项；因此不再另留一个“前缀一致性”假设。
\end{remark}
...
\end{equation}
相应素数积平方自由，可作为 Type II 变量。使用命题
\ref{prop:certificate-zh} 的规范 $1200\times300\times160$ 网格。碰到
Type II 端点或优先级边界的格子直接赋予不利方向下界（若无其他下筛则取零）；
所以其损失已经包含在认证节约中，不再留到网格极限。其余三素数子项用维数一
线性筛下界，并在两种可用下界中取较大者。
...
\begin{lemma}[递归候选的非负性]\label{lem:recursive-candidate-nonnegative-zh}
在上述定义域内，$C_{\rm rec}(\beta_1,\beta_2)\ge0$。
\end{lemma}

\begin{proof}
对前缀 $q_1q_2$、切点 $x^\gamma$ 的模型和使用 Buchstab 恒等式。其基项等于
$x^\gamma<q_3<q_2$ 的孩子之和加上非负的 $q_2$-粗糙余项。分别记归一化模型
贡献为 $B,C,R$，则 $B=C+R$ 且 $R\ge0$。基本命题 II 与
\eqref{eq:omega-bounds-zh} 给出 $B\le u_+/\gamma$，而
\eqref{eq:A3lower-zh} 给出
\[
 C\ge\int_\gamma^{\beta_2}
 W_3(\beta_1,\beta_2,\beta_3)\frac{d\beta_3}{\beta_3^2}.
\]
所以 $C_{\rm rec}\ge B-C=R\ge0$。这是解析模型项之间的不等式；有限程序中的
显式非负检查只是防止向外取整造成过证的保护条件。
\end{proof}
...
\]
误差在固定平移格上一致。命题~\ref{prop:certificate-zh} 后将显式比较该固定余量
包络与 $H(\alpha)$；端点预算不使用未量化的网格极限。
\end{proposition}
...
\begin{remark}[穷尽分支覆盖]\label{rem:branch-coverage-zh}
补充的分支覆盖审计仅使用整数比较检查完整规范格子。它访问
$34\,215\,168$ 个有序 $A_3$ 盒和
...
\[
 ...
 -A_{3,i}^{-}-T_i^{-})\}.
\]
规范网格有 $727$ 个正 $\alpha$ 格。未缩放节约总和为
$465165906793678$；除以 $12N=14400$ 后向下取整得到
$32303187971>32000000000$，即 $0.032303187971>0.032$。

同一整数算法由固定宽度 C++、任意精度 Python 与 Lean 独立实现。Lean 用
\texttt{native\_decide} 计算四个网格块，随后由内核检查总和、除法、有理归一化
和端点比较；C++ 与 Python 独立重建每个格。两套规范逐格输出逐字节一致，
SHA-256 为
\[
\begin{aligned}
...
\end{aligned}
\]
完整命令、块投影、未定义行为检查和诊断性细网格均置于公开验证归档；它们不构成
定理的额外输入。
\end{proof}

\begin{lemma}[显式解析余量预算]\label{lem:explicit-margin-zh}
令 $H_t$ 表示在分支账本中使用 \eqref{eq:margin-parameters-zh} 的平移参数 $t$
所得包络（仅在 $7/6<\alpha<61/50$ 使用平移；其外令 $H_t=H$），并令
\[
 \Delta_t:=\int_{7/6}^{5/4}\max\{0,4\alpha-H_t(\alpha)\}\,d\alpha.
\]
当 $0\le t\le10^{-20}$ 时，
\begin{equation}\label{eq:explicit-margin-loss-zh}
 |\Delta_t-\Delta|\le10^{11}t.
\end{equation}
因此固定取
\begin{equation}\label{eq:explicit-nu-zh}
 \nu_0=10^{-20},
\end{equation}
...
\begin{proof}
证书只在 $7/6<\alpha<61/50$ 上取非零贡献。在此区间
\[
...
\]
所有积分变量至少为 $1/50$。另外
$|\log(\tau_t/\gamma_t)|<\log4<2$、$0\le W_3\le1$，并且
$0\le U_2\le U_{{\rm LS},t}\le40$。三素数与二素数积分的绝对密度分别不超过
\[
 \frac{61}{50}\,50^4<8\cdot10^6,\qquad
 \frac{61}{50}\,40\,50^2<2\cdot10^5,
\]
而 \eqref{eq:H-zh} 的初等基项小于 $400$。这些是在单位盒上的显式可积优函数。

离开分支边界后，对各有理分支关于 $t$ 求导。由
$|\Phi'|\le1$、$|d s_{3,t}/dt|=1/\beta_3\le50$ 以及
\[
 \left|\frac d{dt}\frac2{1/2-\beta_1-\beta_2-t}\right|\le800,
\]
令
\[
...
\]
对递归候选，由 Leibniz 公式和
$|\partial_tW_{3,t}|\le1/\beta_3$，
\[
 |\partial_t C_{{\rm rec},t}|
 \le\frac4{\gamma_t^2}+\frac4{\gamma_t^2}
 +\int_{\gamma_t}^{\beta_2}\frac{d\beta_3}{\beta_3^3}
 <3\cdot10^4.
\]
结合 $U_{{\rm LS},t}$ 的 $800$ 界、与 $t$ 无关的直接 Type II 候选及
$\alpha/(\beta_1\beta_2)\le(61/50)50^2$，完整二素数密度的变化小于
$10^8t$。对这一组处处定义的固定候选取最大、最小或正部时，Lipschitz 常数
不增大；引理~\ref{lem:recursive-candidate-nonnegative-zh} 正好补上此前依赖候选
可用性的唯一一点。

再处理移动的指标与积分端点。四个子集和对两个 Type II 端点产生 $8$ 个超平面，
二素数 Type II 区间产生 $2$ 个，递归 $\xi_t$ 测试产生 $1$ 个，移动的
$\gamma_t$ 或 $\tau_t$ 积分端点出现 $5$ 次；即使重复计算优先级也少于 $32$
个移动仿射超平面。每个右端最多移动 $5t$；因某个系数为 $1$，其在单位盒内的
对称差测度至多 $10t$。
乘以上述 $8\cdot10^6$ 优函数，全部移动薄层的贡献小于
$32\cdot10t\cdot8\cdot10^6<3\cdot10^9t$。加上光滑变化且利用
$\alpha$ 区间长度小于 $1$，总量严格小于 $10^{11}t$。函数
$r\mapsto\max(0,r)$ 为 $1$-Lipschitz，故得到
\eqref{eq:explicit-margin-loss-zh}。

这里没有隐藏网格极限损失。规范网格的 $\alpha$ 宽度为 $1/14400$，三素数
宽度至多 $1/3000$，尾项 $\beta_1$ 宽度至多 $1/2000$，包含孩子宽度至多
$9/8000$。命题~\ref{prop:certificate-zh} 已对每个盒使用不利方向值，包括碰到
Type II 或优先级边界的盒。因此锁定值 $32303187971$ 已是舍弃边界后的下界；
其后唯一损失是 \eqref{eq:explicit-margin-loss-zh}。

最后，$4\nu_0<1/25\le\gamma$ 且
$5\nu_0<1/25\le\sigma-a$，故所有仿射内点条件成立。最后的有理比较也由有限
Lean 审计核对。保留端点裕度的另一半，再取充分大的 $x$ 吸收幂次节约与
$o(1)$ 项。
\end{proof}
...
\]
单独出现的 $1/6$ 保持不变：它是 Merikoski 已发表的
$\alpha\le7/6$ 部分中的基线项，与另列的缺陷项 $F_1$--$F_4$ 分开
\cite[Section 2.6，定理 2 的证明]{Merikoski2020}。
结合引理~\ref{lem:F6-zh} 与命题~\ref{prop:certificate-zh}，零余量认证模型在
$5/4$ 以下的归一化贡献严格小于
...
\end{align}
在固定解析余量 $\nu_0$ 下，引理~\ref{lem:explicit-margin-zh} 使上界至多增加
$10^{-9}$，小于上述余量的一半。再取充分大的 $x$，使全部幂次节约、基本引理和
光滑分片误差合计小于另一半，最终归一化总量仍严格小于 $1$。
...
\eqref{eq:core-zh}、\eqref{eq:tail-zh} 及引理~\ref{lem:explicit-margin-zh}，可得严格小于
...
Lean 4（Mathlib v4.22.0）核查有限 Buchstab 符号、不利方向格子几何、向外取整
规则、$1200\times300\times160$ 整数证书、初等 $F_6$ 中点界、端点算术和
dyadic 实幂换算。四个大块值由 \texttt{native\_decide} 计算，并由固定宽度
C++ 与任意精度 Python 独立重算；最终求和、除法和端点比较由内核检查。证书不
使用浮点或专有计算机代数系统。

Lean 不证明 Grimmelt--Merikoski Type I/II 估计、维数一线性筛、解析
Perron/Mellin 局部化或主定理。这些解析论证写在第
\ref{sec:transfer-zh}、\ref{sec:switching-zh} 节及附录 A 中，仍需通常的数学审稿。

源码、复现命令、锁定工具链和完整审计账本见
\href{https://github.com/CGandGameEngineLearner/landau-fourth-problem-1.323}
{公开验证仓库}；不可变版本由
\href{https://doi.org/10.5281/zenodo.22140874}{Zenodo concept DOI} 标识。

\paragraph{AI 辅助开发。}
GPT-5.6 Sol 作为推理和编码助手参与递归切换论证、证明方向账本、证书实现、审计
脚本和论文表述。作者审查最终数学陈述并对提交内容负责；该模型不是作者，也不是
独立验证者。
...
\begin{lemma}[带前缀模型筛的一致性]\label{lem:uniform-prefixed-model-zh}
对所有 $j\le3$ 的有序前缀格及 $z=q_j$，序列 $\Bcal_u^{\circ}$ 与无前缀
模型具有相同的维数一局部密度和基本引理常数。对层级 $D$ 的 Rosser 权，
\begin{align*}
 \sum_{d\mid P(z)}\lambda_{d,\boldsymbol q}^{+}|\Bcal_{u,d}^{\circ}|
 &\le X_u^{\circ}V(z)
...
\end{align*}
其中常数与具体前缀无关，且
$\sum_{\boldsymbol q}c(\boldsymbol q)E_u=o(\mathfrak M(P))$。用完整
$\Bcal_u$ 替换 $\Bcal_u^{\circ}$，对前缀求和后的改变为
$O(x^{-\gamma_\nu+o(1)}\mathfrak M(P))$。
\end{lemma}
...
\[
 ...
 \end{cases}
\]
而 $p=2$ 吸收到固定初始因子中。因 $u$ 的每个素因子均不小于 $z=q_j$，
$p<z$ 时没有 Euler 因子被移除。因此 $g_u(p)=g(p)$，且
\[
...
\]
这正是 \cite[Theorem 11.12 and Chapter 12]{FriedlanderIwaniec2010}
使用的维数一乘积假设。

再核对光滑族。令 $Y=P/u$、
$W_u(m)=\psi_P(um)\log(um)$。在 $m\asymp Y$ 上，对任意固定 $r$，
\[
 Y^r|W_u^{(r)}(m)|\ll_r\log P,
\]
常数与 $u$ 无关。提出无害的 $\log P$ 后，这正是 Merikoski 引理 8 证明中的
固定半范数。因此定理 11.12 与第 12 章基本引理给出所述两式，常数不依赖
$q_j$ 在其 dyadic 格中的具体取值。

最后，若 $(m,u)>1$，则至多三个前缀素数之一满足
$q_i\mid m$，且 $q_i\ge x^{\gamma_\nu}$。写 $m=q_im'$ 并用
$\rho(n)\ll\tau(n)$，对有序前缀格求和得到
\[
...
把引理~\ref{lem:uniform-prefixed-model-zh} 的可忽略互素修正吸收到 $E_u$，并恢复
$X_u=|\Bcal_{u,1}|$ 的记号。
...
\]
缩放 $b$ 得到 $\psi_2$，固定分解 $\psi_P$ 得到 $\psi_1$，分部求和产生
$\log k$，故它恰为前述 $r_P(e)$。Corollary 7.1 因而给出
\[
 \sum_{e\le x^{1/2-\nu}}|r_P(e)|
...
vol.~57 of American Mathematical Society Colloquium Publications,
...
vol.~33 of London Mathematical Society Monographs Series,
...
Forum Math. Pi \textbf{14} (2026), Paper No.~e8, 54 pp.,
