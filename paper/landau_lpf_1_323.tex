...
\keywords{greatest prime factor, quadratic polynomial, Harman sieve, Buchstab identity, Landau's fourth problem, computer-assisted proof}
...
bounds for the same Buchstab decomposition \cite{Li2025}.\footnote{Carella's
isolated $3/2$ claim is not used: its transition (5.3)--(5.4) replaces the
full von Mangoldt divisor identity by $\log\operatorname{rad}(m)$, losing
prime-power contributions \cite[Section 5]{Carella2025}.}
...
argument used the linear sieve, in the spirit of Harman's sieve
\cite{Harman2007}.  Merikoski already observed that Buchstab's
...
The canonical grid has $727$ positive $\alpha$-cells, and
\[
 \begin{aligned}
 \text{unscaled saving sum}&=465165906793678,\\
 \left\lfloor\frac{465165906793678}{14400}\right\rfloor
 &=32303187971>32000000000.
 \end{aligned}
\]
Thus the certified saving is $0.032303187971>0.032$.

The same integer algorithm is implemented independently in C++, Python, and
Lean.  Lean's native evaluator computes four grid blocks; kernel elaboration
then checks the final sum, division, rational normalization, and endpoint
comparisons.  The C++ and Python programs reconstruct every cell independently.
Their canonical cell dumps agree byte for byte and have SHA-256
\[
 \begin{aligned}
 ...
 \end{aligned}
\]
Full commands, block projections, sanitizer checks, and diagnostic finer
grids are provided in the public verification archive; none is an additional
...
\cite[Section 2.6, proof of Theorem 2]{Merikoski2020}.
...
\section{Verification, data availability, and declarations}

The finite Buchstab signs, proof-adverse cell geometry, outward-rounding
rules, $1200\times300\times160$ integer certificate, elementary $F_6$
midpoint bound, endpoint arithmetic, and dyadic real-power conversion are
checked in Lean 4 with Mathlib v4.22.0.  The four large block values use
\texttt{native\_decide} and are independently recomputed by fixed-width C++
and arbitrary-precision Python; the final sum, division, and endpoint
comparisons are kernel checked.  No floating-point or proprietary
computer-algebra calculation enters the certificate.

Lean does not prove the Grimmelt--Merikoski Type~I/II estimates, the
dimension-one linear sieve, the analytic Perron/Mellin localization, or the
main theorem.  Those analytic arguments are given in
Sections~\ref{sec:transfer}, \ref{sec:switching}, and Appendix~A and remain
subject to ordinary mathematical refereeing.

Source code, reproduction commands, the locked toolchain, and the complete
audit ledger are available in the
\href{https://github.com/CGandGameEngineLearner/landau-fourth-problem-1.323}
{public verification repository}; the immutable release is identified by the
\href{https://doi.org/10.5281/zenodo.22140874}{Zenodo concept DOI}.

\paragraph{AI-assisted development.}
GPT-5.6 Sol was used as a reasoning and coding assistant for the recursive
switching argument, proof-direction ledger, certificate implementation,
audit scripts, and exposition.  The author reviewed the final mathematical
statements and takes responsibility for the submitted content.  The model is
neither an author nor an independent verifier.
