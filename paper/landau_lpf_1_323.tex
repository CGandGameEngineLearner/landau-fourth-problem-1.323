...
Fix \(\boldsymbol q=(q_1,\ldots,q_j)\), \(j\le3\), put
\(u=q_1\cdots q_j\), and let \(w_{\boldsymbol q}\) be the smooth cofactor
localization.  Define
\[
 a_{\boldsymbol q}(m)
 :=|\Acal_{um}|\psi_P(um)\log(um)w_{\boldsymbol q}(m),\qquad
...
\[
 ...
 \psi_P(um)\log(um)w_{\boldsymbol q}(m),\qquad
 X_{\boldsymbol q}:=|\Bcal_{\boldsymbol q,1}|.
\]
The Type-I discrepancy is
...
the least prefix prime, so \((u,d)=1\).  The Euler-product calculation of
\cite[Lemma 8]{Merikoski2020} and
\cite[Theorem 11.12]{FriedlanderIwaniec2010} therefore has the same
dimension-one local factors as the unprefixed sequence:
\[
 V_{\boldsymbol q}(z)
...
The Rosser inequalities, followed by the model Euler-product evaluation,
give \cite[Chapter 12, (12.12)--(12.14), pp. 235--236]{FriedlanderIwaniec2010}
\begin{align}
...
\[
 ...
 |r_{\boldsymbol q}(d)|.
\]
The upper formula is used directly for \(s_{\boldsymbol q}\ge1\); the lower
formula is used for \(s_{\boldsymbol q}\ge2\), and zero is used below \(2\).
...
\[
 D=x^{1/2-s_0-\nu},\qquad ud\le x^{1/2-\nu}.
\]
After the prefix-dependent inner weights have been separated by
Lemma~\ref{lem:localization}, summing the prefix inequalities and collecting
\(e=ud\) gives
\begin{equation}\label{eq:collected-prefix-remainder}
 \mathcal R^{\pm}
 =\sum_{e\le x^{1/2-\nu}}\Lambda_e^{\pm}r(e)+O_A(x^{-A}),\qquad
 \Lambda_e^{\pm}:=
 \sum_{\substack{\boldsymbol q,d:\,ud=e}}
 c(\boldsymbol q)\lambda_{d,\boldsymbol q}^{\pm}.
\end{equation}
This separation is essential: before it, \(r_{\boldsymbol q}(d)\) need not
depend only on \(e\).
...
\]
Corollary~7.1 of \cite{GrimmeltMerikoski2025}, with \(a=h=1\) and
\(\eta=\nu\), gives
...
remainder is admissible before \(\nu\to0^+\).
...
\]
whose sieve parameter is \(1/(1-\omega)>1\).
Letting \(\omega\to0^+\) gives the first term in
\[
 U_{{\rm LS},\nu}(\beta_1,\beta_2)
...
\[
 ...
 =\frac{1/2-s_0-\nu}{\zeta}+O(1/\log x).
\]
Merikoski's Lemma~7 converts the summed model terms to
\eqref{eq:A3lower} and the \(U_{\rm LS}\) term in \eqref{eq:Tlower}.
The prefix-dependent Rosser weights, the collected use of Corollary~7.1,
and this model-integral passage remain external analytic inputs.
