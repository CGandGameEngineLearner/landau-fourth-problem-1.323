...
and their finite Mellin--Perron transfer as external inputs.  This is a quantitative weak form of
...
outward-rounding rules, affine interior-margin identities, half-integer
separation and abstract representation bounds, proof-adverse cell geometry,
model-normalization algebra, the elementary $F_6$ midpoint/convexity
certificate, the exact rational endpoint budget, and the final real-power
conversion; it should not be
...
\begin{proof}
Insert the upper or lower Rosser weights $\lambda_d^\pm$, supported on
$d\le D$ and composed of primes below $z$.  If the prefix product is $u$,
then $ud\le x^{1/2-\nu}$ and $(u,d)=1$.  On collecting equal total moduli
$e=ud$, the number of representations is bounded by a fixed divisor
function (the prefix has at most three prime factors), while
$|\lambda_d^\pm|\le1$.  Corollary~7.1 of
\cite{GrimmeltMerikoski2025} bounds the sum of the absolute discrepancies
over these total moduli.  Multiplication by the fixed divisor bound and the
$O((\log x)^{O(1)})$ dyadic localizations costs $x^{o(1)}$, which is
absorbed by its power saving.  The model sequence is multiplicative on the
coprime prefix and Rosser variables.  If $z$ is the least prefix prime, the
Rosser weight may depend on the particular prefix representation; after
collecting $e=ud$, its absolute contribution is still bounded by the same
fixed divisor function because $|\lambda_d^\pm|\le1$.  Thus the absolute
Type~I discrepancy sum supplies exactly the required uniformity, without a
fixed weight shared by all prefixes.  The usual fundamental lemma of the
dimension-one linear sieve then gives the asserted $F$ and $f$ factors.  The
conversion of the remaining prime sums to Buchstab integrals is the same as
\cite[Lemma 7]{Merikoski2020}.
\end{proof}
