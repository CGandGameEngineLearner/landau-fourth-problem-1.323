...
subtracted one-prime term, this gives
\[
  \Pplus(n^2+1)>n^{1.323}
\]
for infinitely many positive integers $n$.  The internal block exponent is
...
final dyadic-to-pointwise real-power inequality.  The displayed endpoint
budget still treats the cited analytic estimates, the Buchstab lower bound,
and their finite Mellin--Perron transfer as external inputs; Lean does not
verify the localization or prefix-uniform sieve transfer.  This is a quantitative weak form of
...
The factors $U^{-s}$ and $(V-1/2)^s$ belong to opposite coefficients.
On this line their absolute values are $O_C(1)$, and the integral kernel
has $L^1$ norm $O(\log T)$.  Moreover
$|V-1/2-U|\ge1/2$ and $U,V\le x^C$, so
$|\log((V-1/2)/U)|\gg_C x^{-C}$.  The number of tuples before applying the
arithmetic estimate is at most $x^{O(1)}$ in the fixed decomposition;
choosing $T=x^{A'}$ with
$A'$ sufficiently large therefore makes the aggregate Perron error
$O_A(x^{-A})$.  Iterating a fixed number of comparisons costs only a fixed
power of $\log x$.
...
lemma.  This argument is the
quantitative version of the cross-condition removals in
\cite[Sections 2.3 and 3.4]{Merikoski2020}.
\end{proof}

\begin{proposition}[Prefix-uniform linear sieve]\label{prop:prefix-sieve}
Let a fixed ordered prefix consist of at most three primes, all at least
$z=x^\zeta$, and let its product have exponent $s_0$.  The cutoff $z$ may
also be the least prefix prime after dyadic localization.  The Type~I estimate
in Proposition~\ref{prop:transfer}, summed over any dyadically localized
family of such prefixes with divisor-bounded weights, permits the standard
dimension-one upper and lower linear sieve with remainder level
\[
 D=x^{1/2-s_0-\nu}.
\]
...
\begin{proof}
For a localized ordered prefix $\boldsymbol q=(q_1,\ldots,q_j)$, $j\le3$,
write $u=q_1\cdots q_j$.  Insert the upper or lower Rosser weights
$\lambda_{d,\boldsymbol q}^{\pm}$ supported on $d\le D$ and on products of
primes below the cutoff $z=z(\boldsymbol q)$.  When the cutoff is the least
prefix prime, $z=q_j$; hence $(u,d)=1$.  Since $u$ has exponent $s_0$,
\[
 ud\le x^{s_0}x^{1/2-s_0-\nu}=x^{1/2-\nu},
\]
which is strictly inside the level in Corollary~7.1 of
\cite{GrimmeltMerikoski2025}.

Let $r(e)$ denote the modulus-$e$ discrepancy appearing in that corollary.
After summing the localized prefixes and collecting equal total moduli
$e=ud$, the remainder has the form
\[
...
\]
For fixed $e$, the ordered prefix has at most three prime slots and $d=e/u$
is then determined.  Thus the number of representations is bounded by a
fixed divisor function.  The localized prefix coefficient
$c(\boldsymbol q)$ is divisor bounded, and
$|\lambda_{d,\boldsymbol q}^{\pm}|\le1$, so
$|\Lambda_e|\ll\tau_K(e)$ for one fixed $K$.  Corollary~7.1 bounds the sum
of the absolute discrepancies $|r(e)|$ through this level.  Since
$\tau_K(e)\ll_\varepsilon e^\varepsilon$, the fixed divisor factor and the
finitely many dyadic localizations are absorbed by its power saving.  This
argument does not require the different prefixes to share one Rosser weight:
the possibly prefix-dependent weights have already been collected into
$\Lambda_e$ before the absolute discrepancy sum is used.

On the model side, $(u,d)=1$ separates the common prefix Euler factor from
the sieve variable.  The standard dimension-one fundamental lemma, used as
an external input, then gives $e^{-\gamma_E}F(s)$ or
$e^{-\gamma_E}f(s)$ with $s=(1/2-s_0-\nu)/\zeta$.  In the $A_3$ lower
branch the prefix is $(q_1,q_2,q_3)$, $z=q_3$, and the lower weight gives the
$\Phi(s_3)$ contribution.  In the two-prime child upper branch the prefix is
$(q_1,q_2)$, $z=q_2$, and the upper weight gives the $U_{\rm LS}$ option.
The conversion of these model prime sums to the displayed Buchstab
integrals uses \cite[Lemma 7]{Merikoski2020}.  Neither the fundamental lemma
nor this application of Corollary~7.1 is formalized in Lean.
\end{proof}
...
Four small audit modules address finite parts of the six analytic-review
gates.  The interior module checks the shifted $\nu$-parameter definitions,
strict interval inclusions, exponent-ledger identities, and cutoff ordering.
The finite-transfer module checks the half-integer separation for integer
strict comparisons and proves an abstract coefficient bound for finite
representations encoded by fixed tuples of divisors.  It does not construct
the actual Rosser/prefix encoding.  The proof-adverse geometry module checks
the common and containing grid endpoints and inclusions, the exact Type~II
and recursive Boolean gates, and the containing tail-child mesh.  The model
normalization module checks the $\alpha/\gamma$ ratio, the $t+1$ lower-sieve
threshold, the rational $\Phi(p/q)$ weight, and both branches of
$U_{\rm LS}$.  These are algebraic, combinatorial, and finite-grid statements;
they do not prove any of the six analytic transfer gates as a whole.
...
endpoint comparisons are then checked without native recomputation.  No
