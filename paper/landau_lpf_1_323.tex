...
outward-rounding rules, and the exact rational endpoint budget.  The displayed
endpoint budget still treats the cited analytic estimates and their finite
Mellin--Perron transfer as external inputs.  This is a quantitative weak form of
...
outward-rounding rules, and exact rational endpoint budget; it should not be
...
The module \texttt{HarmanRecursiveCertificate.lean} is a third,
kernel-checked implementation of the complete fixed-point algorithm.  Lean
evaluates the canonical grid by \texttt{native\_decide} in four 192-cell
...
computation to its exact natural-number value, after which
\texttt{HarmanRecursiveCertificateCanonical.lean} proves the final sum and
strict target inequality without re-expanding the computations.
...
prove the displayed lower bound independently.
\end{proof}
...
elaboration memory bounded.  The endpoint module checks only the exact
rational comparisons, including
$1323/1000<13231/10000$ and the strict normalized budget.  The focused project
builds with Lean 4 and Mathlib v4.22.0.

The formal development treats the published analytic estimates and their
Type II/Perron application in Lemma \ref{lem:A3-transfer}, together with the
standard upper and lower linear-sieve theorem, as analytic inputs.
The finite Buchstab decomposition, the complete integer computation, its
primitive outward-rounding rules, and the exact rational endpoint arithmetic
are checked in Lean.  The previously used ten-panel $F_6$ midpoint,
convexity/Jensen argument, and real-power dyadic conversion are not part of
the focused Lean project.  No floating-point or proprietary computer-algebra
calculation is an input to the finite certificate.
...
combinatorial, integer, rounding, and rational endpoint components; the cited
